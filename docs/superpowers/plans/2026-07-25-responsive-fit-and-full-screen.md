...
**Architecture:** A pure orientation helper selects which pane needs fitting. A reusable injected-style script preserves Oxford's 320px natural width and dynamically scales the complete page below 100% when its pane is narrower. The root keeps only horizontal safe-area edges, hides the system status bar, and gives the native alphabet index explicit top and bottom safe padding.
...
- Calculate zoom from the current pane width; do not use an iPhone-specific
  fixed ratio or clip the 320px page.
...
`oxford-viewer-pane-width-fit`, reads the current document width, calculates
`Math.min(1, paneWidth / 320)`, keeps `body` at a 320px minimum width, and
applies the calculated CSS `zoom`. The disabled script removes the same style
element. Assert both scripts end in `true;`.
...
Use `height >= width` as portrait. The injected script must calculate:

```js
var paneWidth = document.documentElement.clientWidth;
var scale = Math.min(1, paneWidth / 320);
```

The generated style preserves `body { min-width: 320px }`, applies
`zoom: <scale>`, compensates body width with `100 / scale`, and hides residual
horizontal overflow on `html`. Enabling reuses or creates the stable style
element. Disabling removes it.
...
Test that the app safe-area view uses only `['left', 'right']` and the Expo
`StatusBar` receives `hidden={true}`. Test the pure orientation helper
separately from Task 1. Render the alphabet plugin with `topInset={59}` and
`bottomInset={34}` and verify its container has 61px top padding and 36px
bottom padding, including the existing 2px visual padding.
...
`SafeAreaView` edges to `['left', 'right']` and set `hidden` on `StatusBar`.
Pass top and bottom insets to the alphabet plugin and combine them with its
existing 2px padding.
...
`.responsive_row: 300px`, plus the orientation-specific below-100% scaling
behavior.
